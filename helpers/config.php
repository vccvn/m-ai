...
if(!function_exists('get_cfg_domain_options')){
    ...
    function get_cfg_domain_options()
    {
        static $domains = [];
        if(!count($domains)){
            $db = ($s = get_setting_domains())?$s: get_cfg_domains();
...
if(!function_exists('get_cfg_domain')){
    ...
    function get_cfg_domain()
    {
        static $domain = null;
        if(!$domain) {
            if($list = ($s = get_setting_domains())?$s: get_cfg_domains()){
                $d = null;
                $server = get_domain();
                foreach($list as $dm){
...
if(!function_exists('webconfig')){
    ...
    function webconfig($name = null){
        ...
        $list = ['system', 'user', 'web', 'general', "ecommerce", "home", 'mailer', 'payments', 'edu', 'crazy3d'];
...
if(!function_exists('get_home_config')){
    /**
     * lấy thông tin cấu hình cho hệ thống tmdt
     *
     * @param string $key
     * @return mixed
     */
    function get_home_config($key = null, $default = null){
        if($home = webconfig('home')){
            if(!is_null($key)) return $home->get($key, $default);
            return $home;
        }
        return null;
    }
...
if(!function_exists('site_contents')){
     ...
     * @return \Gomee\Helpers\Arr|string|float|integer
     */
    function site_contents($key = null, $default = null){
        static $siteinfo = null;
        if(!$siteinfo) $siteinfo = get_setting('contents');
        if(!$siteinfo) return $default;
        if(is_null($key)) return $siteinfo;
        return $siteinfo->get($key, $default);
    }
...
if(!function_exists('general_setting')){
    ...
    function general_setting($key = null, $default = null){
        static $siteinfo = null;
        if(!$siteinfo) $siteinfo = get_setting('general');
        if(!$siteinfo) return $default;
        if(is_null($key)) return $siteinfo;
        return $siteinfo->get($key, $default);
    }
...
if(!function_exists('discount_setting')){
     ...
     * @return \Gomee\Helpers\Arr|string|float|integer
     */
    function discount_setting($key = null, $default = null){
        static $siteinfo = null;
        if(!$siteinfo) {
            $siteinfo = get_setting('discounts');
            if(!$siteinfo->non_ref_discount && $siteinfo->mon_ref_discount != 0){
                $siteinfo->non_ref_discount = $siteinfo->mon_ref_discount;
            }
        }
        if(!$siteinfo) return $default;
        if(is_null($key)) return $siteinfo;
...
if(!function_exists('trekka_setting')){
     ...
     * @return \Gomee\Helpers\Arr|string|float|integer
     */
    function trekka_setting($key = null, $default = null){
        static $siteinfo = null;
        if(!$siteinfo) $siteinfo = get_setting('trekka');
        if(!$siteinfo) return $default;
        if(is_null($key)) return $siteinfo;
        return $siteinfo->get($key, $default);
    }
...
if(!function_exists('get_home_setting')){
     ...
     * @return \Gomee\Helpers\Arr|string|float|integer
     */
    function get_home_setting($key = null, $default = null){
        static $home = null;
        if(!$home) $home = get_setting('home');
        if(!$home) return $default;
        if(is_null($key)) return $home;
        return $home->get($key, $default);
    }
}
...
if(!function_exists('home_setting')){
     ...
     * @return \Gomee\Helpers\Arr|string|float|integer
     */
    function home_setting($key = null, $default = null){
        return get_home_setting($key, $default);
    }
}
...
if(!function_exists('get_setting_domains')){
    function get_setting_domains() {
        $domain = web_setting('domain');
        $aliasDomain = web_setting('alias_domain');
        $list = [];
        if($domain)
            $list[] = $domain;
        if($aliasDomain)
