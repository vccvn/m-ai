...
if(!function_exists('get_merchant_menu')){
    /**
     * lấy về mảng thông tin menu
     * @param string $filename
     * @return array
     */
    function get_merchant_menu($filename = null){
        // DynamicPost::check($request);
        if(!$filename) $filename = 'merchant';
        $menu = new MenuEngine('merchant/menus');
        $menu->disableCheck = true;
        return $menu->get($filename);
...
if(!function_exists('merchant_action_menu')){
    /**
     * khai báo breadcrumbs
     * @param array $breadcrumbs
     * @return array
     */
    function merchant_action_menu($actionmenus = null){
        $menus = get_web_data('merchant_action_menu')??[];
        if(is_null($actionmenus)) return $menus;
        if(is_array($actionmenus)) $menus = array_merge($menus, $actionmenus);
        set_web_data('merchant_action_menu', $menus);
        return true;
    }
}
...
if(!function_exists('set_merchant_template_data')){
    function set_merchant_template_data($list_name, $template)
    {
        $data = get_merchant_template_data($list_name);
        if(!in_array($template, $data)) $data[] = $template;
        set_web_data('merchant_templates.'.$list_name, $data);
    }
}

if(!function_exists('get_merchant_template_data')){
    function get_merchant_template_data($list_name)
    {
        if(!is_array($data = get_web_data('merchant_templates.'.$list_name, []))) $data = [];
        return $data;
    }
}
