...
if (!function_exists('get_payment_packages')) {
    ...
    function get_payment_packages($args = [])
    {
        ...
        if (!$packages) {
            ...
            if (($user = auth()->user()) && $user->agent_id && $agent = app(UserRepository::class)->first(['id' => $user->agent_id, 'type' => User::AGENT])) {
...
if (!function_exists('get_client_payment_packages')) {
    /**
     * lay thong tin goi
     *
     * @param array $args
     * @return UploadPackage[]
     */
    function get_client_payment_packages($args = [])
    {
