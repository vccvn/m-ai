...
class PostController extends AdminController
{
    ...
    /**
     * @var MetadataRepository $metedatas
     * Quản lý meta data
     */
    protected $metedatas;
    ...
    public function __construct(
        ...
        if (admin_check_dynamic()) {
            ...
            if ($dynamic->post_config) {
                $this->postConfig = new Arr(is_array($dynamic->post_config)?$dynamic->post_config:json_decode($dynamic->post_config, true));
            }
        ...
        if (!$this->postConfig) {
            $this->postConfig = new Arr([]);
        }
        
        if ($this->postConfig && $this->postConfig->hasAny(["thumbail_width", "thumbail_height", "social_width", "social_height"])) {
            if ($this->postConfig->thumbnail_width && $this->postConfig->thumbnail_width > 0) {
                $this->thumbWidth = $this->postConfig->thumbnail_width;
            }
            if ($this->postConfig->thumbnail_height && $this->postConfig->thumbnail_height > 0) {
                $this->thumbHeight = $this->postConfig->thumbnail_height;
            }
            if ($this->postConfig->social_width && $this->postConfig->social_width > 0) {
                $this->socialImageWidth = $this->postConfig->social_width;
            }
            if ($this->postConfig->social_height && $this->postConfig->social_height > 0) {
                $this->socialImageHeight = $this->postConfig->social_height;
            }
        }
    ...
    /**
     * can thiệp trước khi đổ ra view form
     *
     * @param Request $request
     * @param Arr $config
     * @param Arr $inputs
    ...
    public function beforeGetCrudForm($request, $config, $inputs, $data, $attribues)
    {
        ...
        if ($inputs->has('featured_image')) {
            $featureImage = $inputs->featured_image;
            $featureImage['data-width'] = $this->thumbWidth;
            $featureImage['data-height'] = $this->thumbHeight;
            $inputs->featured_image = $featureImage;
        }
