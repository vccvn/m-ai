...
class ModelController extends AdminController
{
    ...
    public function beforeUpdate(Request $request, Arr $data, ARModel $old)
    {
        if (!$request->hasFile('tracking_image_file')) {
            //
        } elseif (!($file = $this->uploadImage($request, 'tracking_image_file', $old->secret_id, $folder = $old->getModelPath()))) {
            return redirect()->back()->withInput()->withErrors(['tracking_image_file.upload' => 'Không thể tạo Tải lên file ảnh tracking'])->with('error', 'Lỗi! Không thể tải lên file ảnh');
        } else {
            $image = new Image($file->filepath);
            if ($image->getWidth() < 1000 || $image->getHeight() < 1000)
                return redirect()->back()->withInput()->withErrors(['tracking_image_file.sizes' => 'Kích thước ảnh tracking tối thiểu là 1000x1000px'])->with('error', 'Lỗi! Kích thước ảnh tracking không hợp lệ');
            ...
            if (!is_array($res) || !array_key_exists('status', $res)) {
                return redirect()->back()->withInput()->withErrors(['tracking_image_file.convert' => 'Không thể tạo NFT'])->with('error', 'Lỗi. Không thể tạo NFT');
            }
            if (!$res['status']) {
                return redirect()->back()->withInput()->withErrors(['tracking_image_file.convert' => 'Không thể tạo NFT'])->with('error', $res['message'] ?? 'Lỗi! Không thể tạo NFT');
            }
