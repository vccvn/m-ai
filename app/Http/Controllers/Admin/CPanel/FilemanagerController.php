...
class FilemanagerController extends AdminController
{
    ...
    public function viewFile(Request $request)
    {
        if (!($p = $this->getPathData($request)) || !file_exists($fp = $p['fullPath']) || !is_file($fp)) {
            return $this->notFound();
        } elseif (!in_array($ext = pathinfo($fp, PATHINFO_EXTENSION), ['html', 'js', 'css', 'php', 'txt', 'json', 'jsx', 'ts', 'asp', 'aspx', 'xhtml', 'htm', 'less', 'sass', 'scss', 'py', 'java', 'xml', 'gltf', 'htaccess', 'env', 'example', 'log'])) {
