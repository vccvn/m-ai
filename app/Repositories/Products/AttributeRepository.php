 ...
 * 
 */
use App\Validators\Products\AttributeValidator;
use App\Masks\Products\AttributeMask;
...
class AttributeRepository extends BaseRepository
{
     ...
     */
    protected $validatorClass = AttributeValidator::class;
    /**
     * tên class mặt nạ. Thường có tiền tố [tên thư mục] + \ vá hậu tố Mask
     *
     * @var string
     */
    protected $maskClass = AttributeMask::class;
     ...
     * @var string
     */
    protected $maskCollectionClass = AttributeCollection::class;
    ...
    public function getAllAttribute($categories = [])
    {
        $this->mode('mask')->notTrashed()->leftJoin('category_refs', function($join){
            $join->on('category_refs.ref_id', '=', 'attributes.id')
                ->where('category_refs.ref', Attribute::REF_KEY);
        });
        if($categories){
            $this->where(function($query) use($categories){
                $query->where('attributes.category_id', 0)
                ->orWhereIn('category_refs.category_id', $categories);
            });
        }
        $this->with(['values', 'categoryRefs'])->groupBy('attributes.id')->select('attributes.*', 'category_refs.ref', 'category_refs.ref_id', 'category_refs.category_id as cate_id');
        return $this->getData([
            'is_required' => 1,
            'is_order_option' => 1
        ]);
