...
class ProductAttributeRepository extends BaseRepository
{
     ...
     * @param array $variant_thumbnails các thumbnails bổ xung
     * @return array
     */
    public function saveAttributeValues(
        ...
        array $default_selected = [],
        array $variant_thumbnails = []
    )
    {
            ...
            foreach ($attributeGroup['variants'] as $variant) {
                if (!isset($variants[$variant->name])) {
                    ...
                    foreach ($inputData as $attribute_value_id) {
                        ...
                        if (isset($variant_thumbnails[$attribute_value_id]) && (strlen($a = $variant_thumbnails[$attribute_value_id]) && $variant->use_thumbnail == 1)) $data['thumbnail'] = $a;
