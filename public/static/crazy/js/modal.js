$(function () {
    ...
    Object.assign(CrazyModal.prototype, {
        constructor: CrazyModal,
        on: function addEventListener(event, callback) {
            if (typeof this.events[event] == "undefined") this.events[event] = [];
            if (typeof callback == "function") {
                this.events[event].push(callback);
            }
        },
        emit: function dispatchEvent(event, target) {
            if (typeof this.events[event] != "undefined") {
                var self = this;
                this.events[event].map(cb => cb.apply(self, [target, $(target).attr('id')]));
            }
        },
        /**
         * Hiển thị modal
        ...
        show: function (id, show_callback, hide_callback) {
            ...
            if (self.is_on) {
                self.nextModal = id;
                self.prevModal = self.current_open;
                self.hide(null, function () {
                    self.show(id, show_callback, hide_callback);
                });
            } else {
                if (id == self.nextModal) self.nextModal = null;
                if (typeof show_callback == 'function') {
                    self.show_callback = show_callback;
                    self.showCallback[id] = show_callback;
                }
                $('#' + id).modal('show');
                ...
                if (typeof hide_callback == "function") {
                    ...
                    setTimeout(function () {
                        self.hideCallback[id] = hide_callback;
        ...
        hide: function (id, callback) {
            var self = this;
            if (typeof callback == 'function') {
                if (id) self.hideCallback[id] = callback
    ...
    if ($('.modal').length > 0) {
        ...
        $('.modal').on('hidden.bs.modal', function () {
            ...
            var $this = this;
            var id = $($this).attr('id');
            App.modal.emit("hide", $this);
            setTimeout(function () {
                if (id && typeof App.modal.hideCallback[id] == "function") {
                    App.modal.hideCallback[id].apply(App.modal, [$this, App.modal]);
                    // App.modal.hideCallback[id] = null;
                    // delete App.modal.hideCallback[id];
                }
                else if (typeof App.modal.hide_callback == 'function') {
                    App.modal.hide_callback($this, App.modal);
                }

                App.modal.last_open = App.modal.current_open;
                App.modal.prevModal = App.modal.last_open;
        ...
        $('.modal').on('show.bs.modal', function (e) {
            ...
            var id = $($this).attr('id');
            App.modal.emit("show", $this);
            setTimeout(function () {
                App.modal.current_open = id;
                if (typeof App.modal.showCallback[id] == "function") {
                    App.modal.showCallback[id].apply(App.modal, [$this, App.modal]);
                    // App.modal.showCallback[id] = null;
                    // delete App.modal.showCallback[id];
                }
                else if (typeof App.modal.show_callback == 'function') {
