...
class P {
  ...
  filter(t, i) {
    ...
    const { xPrev: e, tPrev: r, dxPrev: o } = this, s = t - r, u = h(s, this.dCutOff), c = [], d = [], l = [];
    for (let a = 0; a < i.length; a++) {
      c[a] = (i[a] - e[a]) / s, d[a] = p(u, c[a], o[a]);
      const m = this.minCutOff + this.beta * Math.abs(d[a]), f = h(s, m);
      l[a] = p(f, i[a], e[a]);
    }
    return this.xPrev = l, this.dxPrev = d, this.tPrev = t, l;
  }
}
const g = `<div class="mindar-ui-overlay mindar-ui-loading">\r
  <div class="loader"/>\r
</div>\r
...
class k {
  constructor({ uiLoading: t, uiScanning: i, uiError: e }) {
    ...
    r.innerText = b, document.head.appendChild(r), t === "yes" ? this.loadingModal = this._loadHTML(g) : t !== "no" && (this.loadingModal = document.querySelector(t)), e === "yes" ? this.compatibilityModal = this._loadHTML(v) : e !== "no" && (this.compatibilityModal = document.querySelector(e)), i === "yes" ? this.scanningMask = this._loadHTML(y) : i !== "no" && (this.scanningMask = document.querySelector(i)), this.hideLoading(), this.hideCompatibility(), this.hideScanning();
  }
  showLoading() {
