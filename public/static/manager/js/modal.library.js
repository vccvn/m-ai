var LibraryModal = function (selector) {
    ...
    this.preview = function (id) {
        if (!id || typeof files[id] == "undefined") {
            App.Swal.warning("Ảnh bạn chọn không họp lệ");
        }
...
$(function () {
            ...
            dropzoneLibrary.on("complete", function (file) {
                if (file.xhr) {
                    if (file.xhr.response) {
                        ...
                        if (response.status) {
                            setTimeout(function () {
                                App.library.prepend(response.data);
    ...
    $(document).on("click", ".input-media .input-group", function (e) {
        ...
        if ($inputMedia.length) {
            ...
            console.log("last open before library " + modalOpened);
            App.library.open({
                modal: modal,
                mode: "one",
                preview: true,
