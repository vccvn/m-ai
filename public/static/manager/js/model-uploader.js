...
function ModelUploader(selector, frmselect) {
    ...
    function addFiles(files) {
        if(window.__upload_total__ <= 0) return App.Swal.warning('Bạn đã hết lượt tải lên.<br />Hãy đăng ký thêm lượt tải file!');
    ...
    function upload(file) {
        ...
        App.api.upload(url, fd, {
            ...
            // editor.engine.updateCanvasSize();
            if (rs.status) {
                window.__upload_total__--;
                $('.upload-file-left').html(window.__upload_total__);
                removeBg()
