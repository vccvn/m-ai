...
function addTimePicker(selector, format){
...
App.location = {
    ...
    changeCountryCode: function(value, text, el){
        if(value != this.regionID){
            ...
            App.htmlSelect.changeOptions('region_id', {"": "Chọn một"});
            App.htmlSelect.changeOptions('district_id', {"": "Chọn một"});
            App.htmlSelect.changeOptions('ward_id', {"": "Chọn một"});
            if(value && value != "0"){
                ajaxRequest(this.urls.region_options, "get", {code: value}, function(res){
                    if(res.status){
                        App.htmlSelect.changeOptions('region_id', res.data);
                    }
                });
            }
        }
...
jQuery(function($){
    ...
    $('input[type="password"]').each(function(i, el){
        $(el).parent().addClass('relative').append('<i class="far fa-eye show-password-icon"></i>');
    });
        ...
        $(document).on("change", ".custom-file-input",function(){
            ...
            if (window.File && window.FileList && files && files.length) {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    if((onc || $(self).parent().hasClass('preview') )&& window.FileReader){
                        (function(file, index, coumt) {
                            ...
                            fileReader.onload = function(f) {
                                ...
                                $(self).parent().find('.img-preview').attr('src', src)
                                // console.log($(self).parent().find('.img-preview'));
                                list.push(data);
                                if(index == coumt){
                                    callback(list);
                                }
    ...
    $(document).on("click", ".click-to-edit", function (e) {
        e.preventDefault();
        $(this).closest(".editable-block").addClass('editing');
        $(this).closest(".editable-block").find('textarea').each(function (i, el) {
            addCkeditor(el);
        })
        return false;
    });
...
function apiRequest(method, url, data, headers, success, fail, error){
    App.api[method].apply(App.api, [url, data, headers]).then(function(rs){
        if(rs.status){
            if(typeof success == "function"){
                success(rs);
            }
            ...
            if(typeof fail == "function"){
                ...
                if(err && typeof err == "object" || Array.isArray(err)){
                    message = Object.keys(err).map(k => err[k]).join("<br>");
                }
                if(message){
                    App.Swal.errorDetail(rs.message, message);
                }
                else if(rs.message){
    ...
    }).catch(err => typeof error == "function" ? error(err): App.Swal.error("Lỗi không xác định"))
}
function apiGet(url, data, headers, success, fail, error){
    return apiRequest("get", url, data, headers, success, fail, error);
}
function apiPost(url, data, headers, success, fail, error){
    return apiRequest("post", url, data, headers, success, fail, error);
}
