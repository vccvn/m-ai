...
@section('content')
    ...
    @php
    $pt = trim($path, '/');
    $download = route($route_name_prefix.'filemanager.download');
    $fl = 0;
                ...
                <div class="m-portlet__head">
                    ...
                    <div class="m-portlet__head-tools">
                        <ul class="m-portlet__nav">
                            ...
                            <li class="m-portlet__nav-item">
                                <a href="{{ route($route_name_prefix.'filemanager.upload.form', ['p' => $path]) }}" data-toggle="m-tooltip" data-placement="top" title data-original-title="Tải lên" class="btn-upload-file btn btn-outline-success m-btn btn-sm"><i class="fa fa-upload"></i> <span class="d-none d-md-inline-block">Tải lên</span></a>
                            </li>
                        </ul>
                    </div>
                </div>
                                    ...
                                    <tbody>
                                        ...
                                        @if ($rp)
                                            ...
                                            <tr class="" id="crazy-item-up" data-name="{{ $plv }}">
                                                ...
                                                <td class="text-center check-col">
                                                    <a href="{{ $r = route($route_name_prefix.'filemanager', ['p' => $plv]) }}">
                                        ...
                                        @if (isset($list) && count($list))
                                            @foreach ($list as $item)
                                                ...
                                                <tr class="" id="crazy-item-{{ str_replace(' ', '-', $item->name) }}" data-name="{{ $item->name }}">
                                                    ...
                                                    <td class="text-center check-col">
                                                        <a href="{{ $r = route($route_name_prefix.'filemanager', ['p' => $rp . $item->name]) }}">
                                                            @if ($item->type == 'file')
                                                        ...
                                                        <div class="dropdown-menu">
                                                            ...
                                                            @if ($item->type == 'file')
                                                                @if (in_array($item->extension, $editor_support_types))
                                                                    <a href="{{ route($route_name_prefix.'filemanager.editor') . '?p=' . $fp }}" title="Sửa" class="dropdown-item">
                                                    ...
                                                    <td class="text-right hide-xs w-100">
                                                        ...
                                                        @if ($item->type == 'file')
                                                            @if (in_array($item->extension, ['html', 'js', 'css', 'php', 'txt', 'json', 'jsx', 'ts', 'asp', 'aspx', 'xhtml', 'htm', 'less', 'sass', 'scss', 'py', 'env', 'htaccess', 'example']))
                                                                <a href="{{ route($route_name_prefix.'filemanager.editor') . '?p=' . $fp }}" data-original-title="Sửa" data-toggle="m-tooltip" data-placement="left" title class="text-accent btn btn-outline-accent  btn-sm m-btn m-btn--icon m-btn--icon-only m-btn--pill m-btn--air">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                <tbody>
                                    <tr>
                                        <td>Dung lượng</td>
                                        <td class="text-right"><span>N/A MB</span></td>
                                    </tr>
                                    <tr>
                                        <td>Đã sử dụng</td>
                                        <td class="text-right">
    ...
    <script>
        window.filemanagerInit = function() {
            App.FileManager.init({
                path: '{{ $path }}',
                urls: {
                    create: '{{ route($route_name_prefix.'filemanager.items.create') }}',
                    move: '{{ route($route_name_prefix.'filemanager.items.move') }}',
                    rename: '{{ route($route_name_prefix.'filemanager.items.rename') }}',
                    unzip: '{{ route($route_name_prefix.'filemanager.items.unzip') }}',
                    delete: '{{ route($route_name_prefix.'filemanager.items.delete') }}',
                    download: '{{ route($route_name_prefix.'filemanager.download') }}',
                    mkdir: '{{ route($route_name_prefix.'filemanager.folders.make') }}',
                    install: '{{ route($route_name_prefix.'filemanager.packages.install') }}',

                    editor: '{{ route($route_name_prefix.'filemanager.editor') }}'
        ...
        window.commandInit = function() {
            App.Commands.init({
                urls: {
                    command: '{{ route($route_name_prefix.'filemanager.command') }}'
                },
...
@section('js')
    ...
    <script>
        $(function() {
            App.api.get("{{ route($route_name_prefix.'filemanager.folders.size') }}").then(function(rs) {
                if (rs.status) {
