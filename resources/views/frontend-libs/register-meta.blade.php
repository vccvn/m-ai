...
@elseif($active_article = get_active_model('post')??(get_active_model('page')??(get_active_model('product')??(get_active_model('project')??get_active_model('course')))))
    @if ($full_title)
        @section('full_title', $full_title)
    @else
...
@elseif(($category = get_active_model('post_category')) || ($category = get_active_model('product_category')) || ($category = get_active_model('project_category')))
    ...
    @endif
    @section('meta_title', $full_title??$category->getFullTitle().' | '.$siteinfo->site_name('Gomee Inc'))
