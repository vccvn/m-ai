...
?>
    ...
    <div {!! $input->attrsToStr() !!} >
        ...
        <div class="dropdown-menu" data-ref="{{$id}}" aria-labelledby="{{$id}}-dropdown">
            ...
            <div class="buttons advance-buttons">
                @if (is_array($advance = $input->hidden('advance')))
                    @foreach ($advance as $item)
                        @if (is_array($item) && $advButton = crazy_arr($item))
                            <div class="adv-item " show="{{$advButton->show('always')}}">
                                <a href="javascript:void(0);" class="btn-advance" data-call="{{$advButton->get('@click')}}">{{$advButton->text}}</a>
                            </div>
                        @endif
                    @endforeach
                @endif
