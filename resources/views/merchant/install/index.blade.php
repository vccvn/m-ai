...
?>
@extends('merchant._layouts.auth', [
    '_base' => 'merchant.',
...
@section('content')
    ...
    <div class="m-grid__item m-grid__item--fluid m-grid m-grid--hor m-login m-login--signin m-login--2 m-login-2--skin-2"
        id="m_login" style="background-image: url({{ asset('static/manager/assets/app/media/img//bg/bg-3.jpg') }});">
        <div class="m-grid__item m-grid__item--fluid	m-login__wrapper">
            <div class="m-login__container">
                ...
                <div class="m-login__signin">
                    ...
                    <form class="m-login__form m-form" action="{{ route('setup.install') }}" method="POST">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group m-form__group">
                            <select name="web_type" id="web-type" class="form-control m-input" style="height: auto">
                                @php
                                    $options = json_decode('{
										"default": "Mặc định",
                                ...
                                @foreach ($options as $value => $text)
									<option value="{{$value}}" @selected($value=='default')>{{$text}}</option>
								@endforeach
                            </select>
                        </div>
                        ...
                        <div class="m-login__form-action">
                            <button id="m_login_signup_submit"
                                class="btn btn-focus m-btn m-btn--pill m-btn--custom m-btn--air m-login__btn m-login__btn--success">Cài
                                đặt</button>
                        </div>
                    </form>
                </div>
