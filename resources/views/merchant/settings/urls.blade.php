...
@section('content')
@if ($urlGroups)
	<div class="m-portlet m-portlet--tabs fit-100vh" id="theme-tabs-list">
...
@if (get_web_data('has_component_area'))
	...
	@section('jsinit')
		<script>
			var component_data = {
				urls: {!! 
					json_encode([
						'sort' => route('merchant.components.sort'),
						'save' => route('merchant.components.ajax-save'),
						'delete' => route('merchant.components.delete'),
						'detail' => route('merchant.components.detail'),
						'inputs' => route('merchant.components.inputs'),
					]) 
				!!},
				list: {!! 
					json_encode([
			...
			posts_data = {
				urls: {
					get_category_url: '{{route('merchant.posts.category-options')}}'
				}
			};
		</script>
	@endsection
	...
	@section('js')
		<?php
		set_merchant_template_data('modals', 'icon-picker-modal');
